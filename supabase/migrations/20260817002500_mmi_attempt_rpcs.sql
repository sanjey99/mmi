...
BEGIN
  ...
  IF p_station_kind = 'standard' THEN
    ...
    WHERE s.station_id = p_station_id AND s.status::TEXT = 'published';
    IF NOT FOUND THEN
    ...
    IF v_prompt_count < 1 OR v_prompt_count <> (
  ...
  ELSE
    ...
    WHERE r.station_id = p_station_id AND r.status::TEXT = 'published';
    IF NOT FOUND THEN
      RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'station_not_found';
    END IF;
    SELECT r.* INTO v_rubric
    FROM public.mmi_scoring_rubrics AS r
    WHERE r.roleplay_station_id = p_station_id AND r.status = 'active';
    IF NOT FOUND THEN
      RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'active_rubric_required';
    END IF;
  ...
  IF p_station_kind = 'standard' THEN
    ...
    LOOP
      INSERT INTO public.mmi_attempt_prompt_snapshots (
        ...
        v_content_version, 'mmi-score-v1', jsonb_build_object('version', 'mmi-score-v1'),
  ...
  ELSE
    INSERT INTO public.mmi_attempt_prompt_snapshots (
      ...
      v_rubric.safety_critical_items, v_content_version, 'mmi-score-v1',
      jsonb_build_object('version', 'mmi-score-v1'), jsonb_build_object('type', 'object')
    );
  END IF;
...
BEGIN
  ...
  IF v_attempt.status = 'completed' THEN
    RAISE EXCEPTION USING ERRCODE = 'P0001', MESSAGE = 'completed_attempt';
  END IF;
