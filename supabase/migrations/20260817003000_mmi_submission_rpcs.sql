...
GRANT ALL PRIVILEGES ON TABLE public.mmi_scoring_provider_attempts TO service_role;
...
BEGIN
  ...
  END IF;
  SELECT COUNT(*), MIN(capacity_at) INTO v_completion_capacity, v_oldest_window_event
  FROM (
    SELECT c.completed_at AS capacity_at FROM public.mmi_scoring_claims AS c
    WHERE c.user_id = p_user_id AND c.status = 'completed' AND c.completed_at >= clock_timestamp() - INTERVAL '24 hours'
    UNION ALL
    SELECT c.completion_reservation_at AS capacity_at FROM public.mmi_scoring_claims AS c
    WHERE c.user_id = p_user_id AND c.status <> 'completed' AND c.completion_reservation_at >= clock_timestamp() - INTERVAL '24 hours'
      AND (NOT v_existing_claim OR c.id <> v_claim.id)
  ) AS capacity;
  IF v_completion_capacity >= 60 THEN
    v_retry_after := GREATEST(1, CEIL(EXTRACT(EPOCH FROM (v_oldest_window_event + INTERVAL '24 hours' - clock_timestamp())))::INTEGER);
    RETURN jsonb_build_object('code', 'rate_limited', 'retryAfter', v_retry_after);
  END IF;
  ...
  IF v_existing_claim THEN
    UPDATE public.mmi_scoring_claims SET status = 'claimed', lease_token = v_lease,
      ...
      completion_reservation_at = COALESCE(completion_reservation_at, clock_timestamp()),
...
DECLARE v_claim RECORD; v_attempt RECORD; v_snapshot RECORD; v_result_id UUID; v_overall NUMERIC; v_completed_count INTEGER;
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN RAISE EXCEPTION USING ERRCODE = '42501', MESSAGE = 'service_role_required'; END IF;
  ...
  PERFORM pg_advisory_xact_lock(hashtextextended(v_claim.user_id::TEXT, 0));
