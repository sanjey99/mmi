...
BEGIN
  ...
  v_criteria:=v_snapshot.rubric_snapshot->'criteria';
  IF NOT FOUND OR jsonb_typeof(p_public_assessment)<>'object' OR p_public_assessment->>'schemaVersion'<>'3' OR jsonb_typeof(p_public_assessment->'criteria')<>'array' OR (SELECT count(*) FROM jsonb_object_keys(p_public_assessment))<>3 THEN RAISE EXCEPTION USING ERRCODE='22023', MESSAGE='invalid_rubric_assessment'; END IF;
  ...
  IF v_actual_ids IS DISTINCT FROM v_expected_ids OR EXISTS (SELECT 1 FROM jsonb_array_elements(p_public_assessment->'criteria') AS item(value) WHERE jsonb_typeof(item.value)<>'object' OR (SELECT count(*) FROM jsonb_object_keys(item.value))<>3 OR jsonb_typeof(item.value->'achieved')<>'boolean' OR jsonb_typeof(item.value->'weightPct')<>'number') THEN RAISE EXCEPTION USING ERRCODE='22023', MESSAGE='invalid_rubric_assessment'; END IF;
  ...
  IF jsonb_typeof(p_usage)<>'object' OR p_usage->>'currency'<>'USD' OR p_usage->>'outcome'<>'scored' OR (p_usage->>'latencyMs')::integer<0 THEN RAISE EXCEPTION USING ERRCODE='22023', MESSAGE='invalid_usage'; END IF;
  ...
  VALUES(v_session.user_id,p_session_id,p_response_id,p_lease_token,p_usage->>'provider',p_usage->>'model',(p_usage->>'inputTokens')::bigint,(p_usage->>'cachedInputTokens')::bigint,(p_usage->>'outputTokens')::bigint,(p_usage->>'inputRatePerMillion')::numeric,(p_usage->>'cachedInputRatePerMillion')::numeric,(p_usage->>'outputRatePerMillion')::numeric,'USD',(p_usage->>'estimatedCost')::numeric,(p_usage->>'latencyMs')::integer,'scored');
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role' THEN RAISE EXCEPTION USING ERRCODE='42501', MESSAGE='service role required'; END IF;
  SELECT response.* INTO v_response FROM public.candidate_mmi_station_responses response WHERE response.id=p_response_id AND response.session_id=p_session_id FOR UPDATE;
  SELECT * INTO v_session FROM public.candidate_mmi_station_sessions WHERE id=p_session_id;
  IF NOT FOUND THEN RAISE EXCEPTION USING ERRCODE='P0002', MESSAGE='candidate_response_not_found'; END IF;
  IF p_usage IS NOT NULL THEN
    INSERT INTO public.mmi_ai_usage_events(user_id,session_id,response_id,lease_token,provider,model,input_tokens,cached_input_tokens,output_tokens,input_rate_per_million,cached_input_rate_per_million,output_rate_per_million,currency,estimated_cost,latency_ms,outcome)
    VALUES(v_session.user_id,p_session_id,p_response_id,p_lease_token,p_usage->>'provider',p_usage->>'model',(p_usage->>'inputTokens')::bigint,(p_usage->>'cachedInputTokens')::bigint,(p_usage->>'outputTokens')::bigint,(p_usage->>'inputRatePerMillion')::numeric,(p_usage->>'cachedInputRatePerMillion')::numeric,(p_usage->>'outputRatePerMillion')::numeric,'USD',(p_usage->>'estimatedCost')::numeric,(p_usage->>'latencyMs')::integer,p_usage->>'outcome');
  END IF;
  ...
  UPDATE public.candidate_mmi_station_responses SET scoring_status='failed' WHERE id=p_response_id AND scoring_status='in_progress';
END;
$function$;
...
BEGIN
  ...
  RETURN (SELECT jsonb_agg(jsonb_build_object('promptOrder',response.prompt_order,'status',response.scoring_status,'legacy',CASE WHEN response.scoring_status='scored' THEN COALESCE((response.public_assessment->>'schemaVersion')::integer,0)<>3 ELSE false END,'assessment',CASE WHEN response.scoring_status='scored' THEN jsonb_build_object('schemaVersion',3,'questionScorePct',response.public_assessment->'questionScorePct','criteria',(SELECT jsonb_agg(jsonb_build_object('criterionId',decision.value->>'criterionId','achieved',(decision.value->>'achieved')::boolean,'weightPct',(decision.value->>'weightPct')::numeric,'bulletText',criterion.value->>'bulletText','domain',criterion.value->'domain') ORDER BY decision.ordinality) FROM jsonb_array_elements(response.public_assessment->'criteria') WITH ORDINALITY AS decision(value, ordinality) JOIN LATERAL jsonb_array_elements(snapshot.rubric_snapshot->'criteria') AS criterion(value) ON criterion.value->>'criterionId'=decision.value->>'criterionId') ) ELSE NULL END) ORDER BY response.prompt_order) FROM public.candidate_mmi_station_responses response JOIN public.candidate_mmi_station_prompt_snapshots snapshot ON snapshot.session_id=response.session_id AND snapshot.prompt_order=response.prompt_order WHERE response.session_id=p_session_id);
END;
$function$;
