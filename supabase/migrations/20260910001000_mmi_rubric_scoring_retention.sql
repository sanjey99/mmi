...
RETURNS boolean LANGUAGE sql IMMUTABLE SET search_path = public, pg_temp AS $function$
  SELECT jsonb_typeof(p_usage) = 'object'
    ...
    AND (
      ...
      OR
      (jsonb_typeof(p_usage->'inputTokens') = 'number' AND jsonb_typeof(p_usage->'cachedInputTokens') = 'number' AND jsonb_typeof(p_usage->'outputTokens') = 'number' AND jsonb_typeof(p_usage->'estimatedCost') = 'number' AND (p_usage->>'estimatedCost')::numeric = (((p_usage->>'inputTokens')::numeric * (p_usage->>'inputRatePerMillion')::numeric + (p_usage->>'cachedInputTokens')::numeric * (p_usage->>'cachedInputRatePerMillion')::numeric + (p_usage->>'outputTokens')::numeric * (p_usage->>'outputRatePerMillion')::numeric) / 1000000))
    )
