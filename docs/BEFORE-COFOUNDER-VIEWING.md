...
- The workbook-derived `questions-part-*.csv` artifacts are local, ignored, private proof only and are not committed or public. Under separate exact approvals, their part-1 and part-2 content was imported as 785 inactive provenance-bearing hosted rows; the next hosted content operation remains separately approval-gated and forward-only. Only the converter, manifest, and provenance/count/hash metadata are commit candidates.
...
- During the current import-idempotency phase, the separately approved `040`, `050`, and exact part-1/part-2 workbook RPC imports were applied as recorded below. The database import capability and all 785 inactive workbook content rows are hosted, but its client workflow has not been deployed to Vercel; publication/candidate-visible rollout and every next content operation remain separately approval-gated and forward-only, as do function deployment, secret/configuration change, user/profile or other application-row DML, Cron, migration-history, and role mutation.
...
### Hosted retry-safe import capability; completed inactive workbook staging
...
- [x] Criteria, model answers, and panel notes are excluded. The CSV artifacts remain local ignored/private proof and are not committed or public; their separately approved part-1/part-2 content is hosted only as inactive rows. Released Practice continues to read the two legacy live rows.
...
Most verification evidence above is local. The import-idempotency database capability and both separately approved workbook content batches are hosted only as inactive drafts, but its Vercel client workflow is not deployed; publication/candidate-visible rollout and any next content operation remain separately exact-approval-gated and forward-only. The separately evidenced UI routing fix, direct-provider scoring fix, and dependency alignment are live as described above.
...
- [x] The separately approved part-1 and part-2 workbook batches were imported on 26 August 2026 through one authenticated-admin RPC transaction each. Their final recorded postflight confirms 785 inactive provenance rows, two ledger rows, unchanged legacy questions, closed direct ACLs, no active imported question, and all expected source IDs. This does not publish content, deploy the Vercel client, complete candidate-visible rollout, or establish overall hosted readiness.
...
11. **Workbook content staging — complete; publication/next content step — pending:** the separately approved part-1 and part-2 RPCs imported all 785 rows from the exact artifacts and source-manifest/workbook SHA recorded above. The final read-only postflight found 785 distinct source IDs, 785 inactive rows, 0 active rows, two ledger entries, unchanged legacy questions, and exact per-batch comparisons. This is not publication, a Vercel-client deployment, candidate-visible rollout, or overall hosted-readiness completion. Any next hosted content operation requires a new exact approval and must be forward-only: never delete imported rows or retry with altered content/a changed batch ID.
