...
| Vitest | 187 passed; mutating integration suites are not part of the default command |
...
2. **Fresh read-only audit:** run `supabase/reconciliation/20260825_hosted_catalog_snapshot.sql` in the hosted SQL Editor, save the returned JSON and MD5, run only the commented migration/Cron follow-up whose relation flag is non-null, and compare catalog, exact RLS definitions, functions, table/column/function grants, triggers, extensions, migration history, and Cron state with the staged scripts. Cron command evidence is digest/length only; never export raw Cron command text because it may contain authorization material. This operation is SELECT-only.
