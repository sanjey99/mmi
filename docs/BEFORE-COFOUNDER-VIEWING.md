...
- [x] Close the candidate security gate before deployment: migration `20260901000000` makes feature disablement stop scoring-provider egress, permits owner-only draft cleanup while disabled, limits checkpoints to five accepted writes per second per session/prompt, caps scoring at three provider attempts while retaining the existing five-minute lease cooldown, and purges expired abandoned/unfinalized transcript drafts as well as finalized transcript text. Migration `20260901001000` installs the least-privilege hourly retention operator. On 1 September 2026, both migrations were first proved in the isolated loopback worktree database with 13/13 candidate integration tests, including expired finalized text and draft deletion through the operator path. They were later applied to the separately isolated hosted preview recorded below; the local proof remains local evidence only.
...
- [x] Provisioned isolated hosted Supabase project `obfwfoykalvoxqdnosus` (`mmi-cofounder-preview`, `ap-southeast-1`, `ACTIVE_HEALTHY`) on 1 September 2026 after Preview Branch creation was rejected on the Free plan. It is a separate no-production-data project, not the shared `tliwifhnsytxpcynuwsy` project and not a Git-linked branch.
- [x] Applied the reviewed 17-version migration history through `20260901001000` from exact branch commit `73e3847e5c50f2cedb3f137b0363a27f8b11c24d` to `obfwfoykalvoxqdnosus`. The blank project first received schema-only definitions for the four hosted-preexisting assessor tables from a read-only parent `pg_catalog` capture (applied SQL SHA-256 `38d4348d102e08a475dd6ea80d9f422f07ece2915c0cb130be14706f82a81e4b`), then the unchanged hosted reconciliation (SHA-256 `3dcdf700aec280e7da3c7523a31c6dd717448b9fca6b265a1cdc3fc5d6dc6aa1`) and remaining reviewed migrations. Candidate SQL hashes: normalized orchestration `871fbcecc505758d4064e69defc4939e0119ab0cc786b85315d17cc278275268`, browser speech `844325bc2d9dcc5a51cb4870a28475936e59fc64189cbdb8b0291a7c2ecc07b7`, hardening `d4d95f27c3970e98f0c5d4373520b0a00494672141c8936110358857f60d6de9`, and retention schedule `a61f73668c9a2636c8d71f949a8096715b5dd439f4b95e1959ef5a463fb456fc`. Seeds, roles, vault secrets, production rows, and Git auto-deployment were excluded.
- [x] On `obfwfoykalvoxqdnosus`, imported the exact flat batches (500 + 285), activated all 785 provenance-bearing rows, imported normalized batches (80/400 + 75/375), and finalized exactly 155 published stations and 775 ordered prompts. Independent postflight at `2026-09-01 02:53:31 UTC` found five 120-second prompts per station, 10 panel rows excluded, two finalized normalized ledgers, feature flag `false`, zero candidate runtime rows, and zero residual preview import operators.
- [ ] Publish exactly one approved active fixed-days privacy notice and provision active clinician-reviewed rubrics for all 775 prompts on `obfwfoykalvoxqdnosus`. The independent postflight found `0` qualifying notices and `0/775` qualifying rubrics. Keep the flag off; legal wording, processor identity, retention days, rubric content, and qualified reviewer identity must not be invented.
...
2. Obtain the approved fixed-days privacy notice and qualified clinician-reviewed rubric set, then publish exactly one notice and 775 active prompt rubrics to isolated preview `obfwfoykalvoxqdnosus`. Current verified counts are `0` and `0/775`; keep the flag off.
3. Configure and deploy the JWT-verified candidate scoring function and server-only provider settings to that same isolated preview. Migration/content deployment is complete; function deployment is not.
4. Confirm the hourly retention job has a successful aggregate-only `cron.job_run_details` entry within two hours of deployment and respond to failures as specified above.
5. Run a named-account end-to-end hosted smoke covering transcript scoring, timing, microphone permission/restart/fallback behavior, safe failures, and the scoring kill switch; deliberately enable the flag only after the smoke is accepted.
6. Create/invite named cofounder accounts and apply exact approved roles.
7. Confirm deep links and a hardened-compatible rollback with dated evidence.
8. Design webcam capture/assessment as the next separately approved workstream; it is not a blocker or hidden capability in this transcript-only release.
...
12. **Isolated candidate preview — schema/content complete, disabled:** project `obfwfoykalvoxqdnosus` is healthy and separate from shared `main`; the 17-version chain ends at `20260901001000`; normalized finalization proves 155 stations/775 prompts and 785 preserved active flat rows; exactly one retention job is configured; the feature flag remains `false`; runtime row count and residual importer count are both zero.
13. **Candidate privacy/rubrics — blocked:** qualifying notice count is `0` and qualifying active clinician-reviewed prompt rubric count is `0/775`. Obtain and separately approve the real legal/clinical artifacts before any insert; never use placeholder approval data.
14. **Candidate function/configuration — pending:** deploy the JWT-verified scoring function and server-only provider settings only to `obfwfoykalvoxqdnosus`, then verify disabled fail-closed behavior before any feature enablement.
15. **Named accounts/roles and hosted smoke:** present each target and every expected bounded write before running it.
...
Do not show the hosted preview to cofounders while any P0 item is unresolved. GitHub's Vercel integration deployed exact commit `b0b2741d24749ca6c037309d885b497cc7082dce` as protected Preview `dpl_Dii7RhSjfNoS35mszaa2R2co7QgD`; an authenticated HTTP smoke returned `200` for the `Interview Station` app shell and its JavaScript bundle. Isolated Supabase project `obfwfoykalvoxqdnosus` now contains the reviewed migration chain through `20260901001000` and finalized 155/775 content with the feature flag off, but it does not contain an approved privacy notice or clinician-reviewed rubrics, and the candidate scoring function/provider configuration is not deployed there. This does not replace interactive device/accessibility QA or authorize feature enablement. Hosted-state claims rely only on exact-target approvals and dated postflight evidence. Webcam is not part of this release decision.
