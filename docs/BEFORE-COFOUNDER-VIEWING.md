...
- The workbook-derived `questions-part-*.csv` artifacts are local, ignored, private proof only and are not committed or public. Under its exact approval, part-1 content was imported as 500 inactive provenance-bearing hosted rows; the part-2 artifact/content remains unimported and hard-gated behind its own exact approval. Only the converter, manifest, and provenance/count/hash metadata are commit candidates.
...
- During the current import-idempotency phase, the separately approved `040`, `050`, and exact part-1 workbook RPC import were applied as recorded below. The database import capability and part-1 content batch are hosted, but its client workflow has not been deployed to Vercel; part 2, function deployment, secret/configuration change, user/profile or other application-row DML, Cron, migration-history, and role mutation remain separately approval-gated.
...
### Hosted retry-safe import capability; completed part 1 and pending part 2
...
- [ ] **Hard gate:** the separately approved 500-row part-1 import is complete; only the exact 285-row part-2 RPC call remains pending separate approval. No broad hosted default ACL change is included; future-object default ACL drift remains a separately approved hazard.
...
- [x] The complete versioned migration chain was executed against a fresh disposable local Supabase database. Earlier exact approvals applied the hosted-only reconciliation, additive migrations `20260825000000`/`01000`/`02000`, final cutover `04000`, import-capability migration `05000`, and the separately approved part-1 import on 26 August 2026. Part 2 remains separately approval-gated.
...
| Independent readiness audits | The prior readiness audit and the follow-up independent read-only audit of this diff are closed with no unresolved Critical or High finding in their respective reviewed scopes. Hosted `040`, `050`, and exact part-1 content import are complete; part 2 remains separately hard-gated. |
...
Most verification evidence above is local. The import-idempotency database capability and the separately approved part-1 content batch are hosted, but its Vercel client workflow is not deployed and part 2 of the workbook remains pending its own exact approval; the separately evidenced UI routing fix, direct-provider scoring fix, and dependency alignment are live as described above.
...
- [x] Migration `20260825005000` was separately approved and applied on 26 August 2026; its recorded Management API preflight/apply/postflight evidence confirms provenance, ledger, RPC, ACL, unchanged-row, and unchanged-default-ACL postconditions. Part 1 is complete as separately approved; part 2 remains its own exact approval-gated operation.
- [x] The separately approved part-1 workbook batch was imported on 26 August 2026 through one authenticated-admin RPC transaction; its recorded postflight confirms 500 inactive provenance rows, one ledger row, unchanged legacy questions, closed direct ACLs, and no part-2 batch. This does not deploy the Vercel client or complete the full workbook migration.
...
11. **Workbook content part 1 — complete; part 2 — pending:** the separately approved part-1 RPC imported `supabase/imports/20260825_med_interview_question_bank/questions-part-1.csv` (SHA-256 `33769d18edf3872fc0b2b43fa957ed309715067a777607388d6c92f851f77c30`) under namespace `med_interview_question_bank`, source-manifest/workbook SHA `903fb1b3eedc92647c5cb9aa48465ebc49deaa618da2a53e3a736667f71d1a71`, and batch `questions-part-1`. It produced 500 inactive rows and ledger fingerprint `129b136a6549abdea2cfd09be9fe1f565563bf1098f598903903dbcacceeb0e5`; the recorded read-only postflight comparison was 500 matched, 0 missing, 0 mismatched, and 0 unexpected. Part 2 is absent and its 285 rows remain a separate exact approval; never retry a timed-out batch with altered content or a changed batch ID. This is not a Vercel-client deployment or full-workbook completion.
