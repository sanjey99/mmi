...
- Local preview verification passes: 35 Node tests, 187 Vitest tests, both coverage systems exceed 80% for lines/branches/functions, TypeScript (including the Edge handler), production web export, and 2/2 fully intercepted Playwright journeys.
...
1. Run `supabase/reconciliation/20260825_hosted_catalog_snapshot.sql` in the hosted SQL Editor, save its structured result and stable catalog hash, run only the commented migration/Cron follow-up whose relation flag is non-null, and compare the evidence with every fail-closed script precondition. Retain only the supplied Cron command digest/length, never raw command text that may embed authorization material.
