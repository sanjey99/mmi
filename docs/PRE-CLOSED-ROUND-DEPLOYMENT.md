...
- The import-idempotency database capability is hosted, while its client workflow remains local on `feat/cofounder-ui-reliability` and is not deployed to Vercel. During this current phase, the separately approved `040`, `050`, and exact part-1 workbook RPC import were applied as recorded below; the part-2 import, secrets/configuration changes, Cron, migration-history, and role mutation remain separately approval-gated.
- Earlier separately approved operations applied the hosted-only reconciliation, additive migrations `20260825000000`, `20260825001000`, and `20260825002000`, final cutover `20260825004000`, import-capability migration `20260825005000`, and part-1 workbook import on 26 August 2026; `score-answer` is v4 ACTIVE and `manage-ai-key` remains v3. Part 2 remains separately approval-gated. The stable Vercel production alias is live at `https://mmi-hazel.vercel.app` from commit `dd0c60b2e6a18bac3494a26a494b1d06a88b2249`; it does not include the client import workflow.
...
- The prior independent readiness audit and the follow-up independent read-only audit of this diff are closed with no unresolved Critical or High finding in their respective reviewed scopes. Hosted `040`, `050`, and exact part-1 content import are complete; part 2 remains hard-gated.
...
- [x] Imported the separately approved part-1 provenance-bearing batch (`questions-part-1`): 500 inactive drafts with an exact private-ledger fingerprint and verified postflight.
- [ ] Keep the part-2 workbook-derived hosted question batch blocked until its exact import operation is separately approved. The Question Desk accepts only provenance-bearing source batches; manual one-question authoring remains on its separate RPC.
