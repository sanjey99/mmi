...
- The current import-idempotency changes exist locally on `feat/cofounder-ui-reliability` and are not deployed. During this current phase, the separately approved `040` was applied as recorded below; beyond it, no hosted `050`, workbook import, secret/configuration change, row DML, Cron, migration-history, or role mutation was performed.
- Earlier separately approved operations applied the hosted-only reconciliation, additive migrations `20260825000000`, `20260825001000`, and `20260825002000`, and final cutover `20260825004000` on 26 August 2026; `score-answer` is v4 ACTIVE and `manage-ai-key` remains v3. Migration `20260825005000` and both workbook imports remain separately approval-gated. The stable Vercel production alias is live at `https://mmi-hazel.vercel.app` from commit `dd0c60b2e6a18bac3494a26a494b1d06a88b2249`; it does not include the current import changes.
...
- The prior independent readiness audit and the follow-up independent read-only audit of this diff are closed with no unresolved Critical or High finding in their respective reviewed scopes. Hosted `040` is complete; `050`, its postcondition verification, and exact-content approvals remain hard-gated.
...
- The preview SQL is split into a hosted-only reconciliation artifact, three additive migrations, final privilege cutover `040`, and local-only pending import migration `050`. The reconciliation, additive migrations `000`/`010`/`020`, and `040` were applied under exact approvals; `050` remains not applied. Beyond the separately approved `040`, no hosted `050`, workbook import, secret/configuration change, row DML, Cron, migration-history, or role mutation occurred during the current local import-idempotency phase.
...
- [ ] Keep the two workbook-derived hosted question batches blocked until `050`, its read-only postconditions, and each exact import operation are separately approved. The Question Desk accepts only provenance-bearing source batches; manual one-question authoring remains on its separate RPC.
...
5. `score-answer` is v4 ACTIVE, `manage-ai-key` remains v3, and the stable Vercel production alias is live at commit `dd0c60b2e6a18bac3494a26a494b1d06a88b2249`; the current import changes are not deployed. Complete a bounded hosted smoke before any further promotion.
...
7. After completed `040`, separately approve `20260825005000_cofounder_question_import_idempotency.sql`. It adds nullable source identity, a private RLS ledger, and a fixed-path authenticated-admin RPC; it performs no workbook import. Verify the unique partial index, ledger ACL/RLS, owner/security-definer/search path, and direct-table denial read-only before content is considered.
