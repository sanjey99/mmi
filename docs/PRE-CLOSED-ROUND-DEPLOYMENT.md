...
- The import-idempotency database capability and all 785 inactive workbook rows are hosted, while its client workflow remains local on `feat/cofounder-ui-reliability` and is not deployed to Vercel. During this current phase, the separately approved `040`, `050`, and exact part-1/part-2 workbook RPC imports were applied as recorded below; publication/candidate-visible rollout, every next content operation, secrets/configuration changes, Cron, migration-history, and role mutation remain separately approval-gated and forward-only.
...
- The preview SQL is split into a hosted-only reconciliation artifact, three additive migrations, final privilege cutover `040`, and import-capability migration `050`. The reconciliation, additive migrations `000`/`010`/`020`, `040`, `050`, and exact part-1/part-2 workbook imports were applied under exact approvals. Publication/candidate-visible rollout, any next content operation, secrets/configuration changes, Cron, migration-history, and role mutation remain unperformed in this phase.
...
- [x] Imported the separately approved provenance-bearing batches: `questions-part-1` (500) and `questions-part-2` (285), for 785 inactive drafts with exact private-ledger fingerprints and verified postflight.
