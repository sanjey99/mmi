...
const assessment = Object.freeze({
  dimensions: {
    ...
    ethics: {
      ...
      applicable: true,
      evidence: null,
      improvement: 'Name escalation triggers.',
    },
    communication: {
      score: null,
      applicable: false,
...
const rpcClient = (results: readonly RpcResult[]) => ({
  rpc: vi.fn().mockImplementation(async () => results[0]),
});
const checkpointResult = Object.freeze({
  sessionId,
  promptOrder: 1,
  draftRevision: 4,
  acceptedAt: '2026-08-26T00:02:20.000Z',
});
const finalizationResult = Object.freeze({
  sessionId,
  promptOrder: 1,
...
describe('candidate MMI API transcript boundary', () => {
  it('accepts exact scenario and terminal projections, including PostgreSQL ISO offsets', async () => {
    ...
    for (const projection of [
      ...
      await expect(
        createCandidateMmiApi(
          rpcClient([{ data: projection, error: null }]),
        ).start(),
  ...
  it('accepts only a current response projection with its exact draft fields and Unicode code-point cap', async () => {
    const api = createCandidateMmiApi(
      rpcClient([{ data: responseProjection, error: null }]),
    );
    await expect(api.start()).resolves.toEqual(responseProjection);
    for (const malformed of [
      { ...responseProjection, draftTranscript: undefined },
      { ...responseProjection, draftRevision: -1 },
      { ...responseProjection, responseStatus: 'closed' },
      { ...responseProjection, rubric: 'private' },
      { ...responseProjection, draftTranscript: 'x'.repeat(12_001) },
      { ...responseProjection, draftTranscript: '😀'.repeat(12_001) },
    ])
      await expect(
        createCandidateMmiApi(
          rpcClient([{ data: malformed, error: null }]),
        ).start(),
  ...
  it('calls and parses exact checkpoint and finalization contracts without sending transcript on finalization', async () => {
    ...
    await expect(
      api.checkpoint(sessionId, 1, 'Manual draft', 4),
    ).resolves.toEqual(checkpointResult);
    client.rpc.mockResolvedValueOnce({ data: finalizationResult, error: null });
    await expect(api.finalize(sessionId, 1, finalizationKey)).resolves.toEqual(
      finalizationResult,
    );
  ...
  it('rejects invalid checkpoint/finalization input and mismatched or expanded RPC results fail closed', async () => {
    const api = createCandidateMmiApi(
      rpcClient([{ data: checkpointResult, error: null }]),
    );
    for (const [candidateSessionId, promptOrder, transcript, revision] of [
      [sessionId, 1, 'x'.repeat(12_001), 1],
      ['not-a-uuid', 1, 'draft', 1],
      [sessionId, 6, 'draft', 1],
      [sessionId, 1, 'draft', -1],
    ] as const) {
      await expect(
    ...
    await expect(
      api.finalize(sessionId, 1, 'not-a-uuid'),
    ).rejects.toMatchObject({ kind: 'invalid_request' });
    await expect(
      createCandidateMmiApi(
        rpcClient([
          { data: { ...checkpointResult, draftRevision: 5 }, error: null },
        ]),
      ).checkpoint(sessionId, 1, 'draft', 4),
    ).rejects.toMatchObject({ kind: 'invalid_response' });
    await expect(
      createCandidateMmiApi(
        rpcClient([
          {
            data: { ...finalizationResult, transcript: 'private' },
            error: null,
          },
        ]),
  ...
  it('parses exactly five ordered feedback rows and rejects private/provider fields recursively', async () => {
    ...
    const malformed = [
      ...
      [
        { ...feedback[0], status: 'scored', assessment: null },
        ...feedback.slice(1),
      ],
    ];
    for (const value of malformed)
      await expect(
        createCandidateMmiApi(
          rpcClient([{ data: value, error: null }]),
        ).feedback(sessionId),
      ).rejects.toMatchObject({ kind: 'invalid_response' });
  });
  ...
  it('maps only exact safe error states without echoing server data', async () => {
    for (const [message, kind] of [
      ['candidate_response_not_open', 'response_closed'],
      ['candidate_response_deadline_not_reached', 'response_not_closed'],
      ['candidate_feedback_not_ready', 'response_not_closed'],
      ['stale_candidate_mmi_checkpoint', 'in_progress'],
    ] as const) {
      await expect(
        createCandidateMmiApi(
          rpcClient([
    ...
    await expect(
      createCandidateMmiApi(
        rpcClient([
          { data: {}, error: { code: 'P0001', message: 'private detail' } },
        ]),
      ).start(),
    ).rejects.toThrow('Candidate MMI is unavailable.');
  });
...
describe('candidate MMI runner finalization boundary', () => {
  function apiFixture() {
    return {
      ...
      finalize: vi.fn().mockResolvedValue(finalizationResult),
  ...
  it('checkpoints only the current response and shares identical in-flight/accepted work without old acknowledgement saving a newer edit', async () => {
    ...
    const checkpointResolvers: Array<(value: CandidateMmiCheckpoint) => void> =
      [];
    api.checkpoint.mockImplementation(
      () =>
        new Promise((resolve) => {
          checkpointResolvers.push(resolve);
        }),
    );
  ...
  it('finalizes once before refresh, shares same-key work, and never advances after a failed finalization', async () => {
    ...
    await expect(
      Promise.all([
        runner.expireCurrentPhase(finalizationKey),
        runner.expireCurrentPhase(finalizationKey),
      ]),
    ).resolves.toEqual([nextResponse, nextResponse]);
    expect(api.finalize).toHaveBeenCalledExactlyOnceWith(
      sessionId,
      1,
      finalizationKey,
    );
    ...
    const failing = apiFixture();
    failing.finalize.mockRejectedValueOnce(
      new CandidateMmiApiError('unavailable'),
    );
    const retryRunner = createCandidateMmiRunner(failing);
    await retryRunner.start();
    await expect(
      retryRunner.expireCurrentPhase(finalizationKey),
    ).rejects.toMatchObject({ kind: 'unavailable' });
    expect(failing.refresh).not.toHaveBeenCalled();
    await expect(
      retryRunner.expireCurrentPhase(finalizationKey),
    ).resolves.toEqual(responseProjection);
  ...
  it('refreshes scenario expiry without finalization and abandons once', async () => {
    ...
    const runner = createCandidateMmiRunner(api);
    await runner.start();
    await expect(runner.expireCurrentPhase(finalizationKey)).resolves.toEqual(
      responseProjection,
    );
  ...
  it('restores through refresh and accepts the server completed transition', async () => {
    const api = apiFixture();
    api.refresh
      .mockResolvedValueOnce(responseProjection)
      .mockResolvedValueOnce(completedProjection);
    const runner = createCandidateMmiRunner(api);
    await expect(runner.restore(sessionId)).resolves.toEqual(
      responseProjection,
    );
  ...
  it('does not let an older same-identity refresh overwrite a newer result', async () => {
    ...
    let resolveFirstRefresh:
      | ((value: CandidateMmiServerProjection) => void)
      | undefined;
    ...
    const newer = {
      ...responseProjection,
      serverNow: '2026-08-26T00:02:30.000Z',
      draftTranscript: 'newer',
      draftRevision: 4,
    };
  ...
  it('rejects a checkpoint outside a current response', async () => {
    ...
    await runner.start();
    await expect(
      runner.checkpoint({ transcript: 'draft', revision: 1 }),
    ).rejects.toMatchObject({ kind: 'response_closed' });
  });

  it('uses a capability-checked finalization key path in the route', () => {
    const route = readFileSync(
      resolve(process.cwd(), 'app/practice/mmi-station.tsx'),
      'utf8',
    );
